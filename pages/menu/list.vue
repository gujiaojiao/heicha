<template>
  <view class="menu-list" :style="{ paddingTop: safeAreaInsets.top + 'px' }">
    ...
    <view class="main-content" :style="{ marginTop: 60 + 'px' }">
      ...
      <view class="content">
        ...
        <view class="category-bar">
          <view v-for="cat in categories" :key="cat.id" :class="['cat-item', cat.id === activeCategory ? 'active' : '']" @click="selectCategory(cat.id)">
        ...
        <view class="menuContent">
          <scroll-view scroll-y="true" class="content-scroll" @scroll="onProductScroll" :scroll-into-view="scrollIntoView" :scroll-with-animation="true">
            <!-- banner -->
            ...
            <view class="product-list">
              <view v-for="categoryGroup in groupedProducts" :key="categoryGroup.categoryId" :class="['category-section', `category-${categoryGroup.categoryId}`]" :id="`category-${categoryGroup.categoryId}`">
                <!-- 分类标题 -->
    ...
    <!-- 点单抽屉 -->
    <OrderDrawer v-if="orderDrawerVisible" :visible="orderDrawerVisible" :product-info="currentProduct" @close="closeOrderDrawer" @confirm="confirmOrder" />
...
function onProductScroll(e: any) {
  ...
  scrollTimer.value = setTimeout(() => {
    ...
    for (let i = 0; i < categorySections.length; i++) {
      ...
      const sectionHeight =
        categoryTitleHeight + section.products.length * productItemHeight;

      if (
        scrollTop >= accumulatedHeight &&
        scrollTop < accumulatedHeight + sectionHeight
      ) {
